class HomeController < ApplicationController
  ...
  #파일 업로드
  def upload_file
    sended_msg = Cloudinary::Uploader.upload(params[:file])
    Uploadfile.create(pubilc_id: sended_msg['public_id'], format: sended_msg['format'], url: sended_msg['url'], resource_type: sended_msg['resource_type'])
    render json: {:link => sended_msg['url']}
  end
  def destroy_file
    id = Uploadfile.where(url:params[:src]).public_id
    sended_msg = Cloudinary::Uploader.destroy(id, options = {})
    render json: sended_msg
  end
  #휴즈넷 복구 알고리즘
